...
describe '投稿のテスト' do
  ...
  describe '投稿画面のテスト' do
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        ...
        fill_in 'list[body]', with: Faker::Lorem.characters(number:15)
        click_button '投稿'
  ...
  describe '詳細画面のテスト' do
    ...
    context 'listの削除テスト' do
      it 'listの削除' do
        cexpect{ list.destroy }.to change{ List.count }.by(-1)
      end
    end
  end
  ...
  describe '編集画面のテスト' do
    ...
    context '編集画面への遷移' do
      it '編集前のタイトルと本文がフォームに表示されているか' do
